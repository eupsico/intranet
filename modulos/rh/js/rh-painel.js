...
export function initrhPanel(user, db, userData) {
    ...
    const views = [
        ...
        {id: 'gestao_vagas', name: 'Gestão de Vagas', roles: ['admin', 'rh'], icon: icons.gestao_vagas},
    ...
    async function loadView(viewName) {
        ...
        try {
            ...
            if (!response.ok) throw new Error(`Arquivo da view não encontrado: ${viewName}.html`);
    ...
    function handleNavigation() {
        ...
        const firstPermittedView = views.find(v => v.roles.some(role => userRoles.includes(role.trim())));
