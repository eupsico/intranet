...
// Versão: 2.5 (Adiciona Dashboard como página inicial)
...
export function initsocialPanel(user, db, userData) {
    ...
    // Adicionado "Dashboard" no início da lista de views
    const views = [
        { id: 'dashboard-servico-social', name: 'Dashboard', roles: ['admin', 'servico_social'], icon: icons.dashboard },
    ...
    function start() {
        ...
        const viewExists = views.some(v => v.id === initialHash && !v.isExternal);

        if (initialHash && viewExists) {
