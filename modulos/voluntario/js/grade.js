...
// Descrição: Lógica para a Grade de Horários administrativa, adaptada para o novo formato modular.

import { collection, query, where, orderBy, getDocs, doc, onSnapshot, setDoc } from '../../../assets/js/firebase-init.js';
...
export function init(db, user, userData) {
    ...
    function renderGrade(tipo, dia) {
        if (!gradeContent) return;
        gradeContent.innerHTML = ''; // Limpa o conteúdo
    ...
    async function start() {
        try {
            ...
            const q = query(usersCollectionRef, where("fazAtendimento", "==", true), orderBy("nome"));
            const querySnapshot = await getDocs(q);
            
            querySnapshot.forEach(docSnap => {
                const prof = docSnap.data();
                listaProfissionais.push(prof);
