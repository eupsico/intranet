...
// Versão: 2.2
// Descrição: Implementa a funcionalidade completa das abas de Mensagens e Disponibilidade.
...
export function init(db, user, userData) {
    ...
    function initMensagens() {
        ...
        // --- Event Listeners dos Botões (Mensagens) ---
    ...
    function initDisponibilidade() {
        ...
        function renderEditView(profData) {
            ...
            const scheduleRowsHtml = (profData.horarios && profData.horarios.length > 0 ? profData.horarios : [{dia:'', horario:'', modalidade:'online', status:'disponivel'}])
            .map(h => `
                <tr>
            ...
            container.querySelector('.add-row-btn').addEventListener('click', () => {
                const newRowHtml = container.querySelector('.edit-table tbody tr').outerHTML; // Clona a primeira linha
