...
async function loadTabContent(tabId) {
    ...
    try {
        ...
        if (!pageResponse.ok) throw new Error(`Falha ao carregar o HTML da aba: ${tabId}`);
        ...
        contentArea.innerHTML = '<p class="alert alert-error">Ocorreu um erro ao carregar esta seção.</p>';
    }
}
...
function handleTabClick(e) {
    if (e.target.tagName === 'BUTTON' && e.target.classList.contains('tab-link')) {
        ...
        document.querySelectorAll('#painel-supervisor-tabs .tab-link').forEach(btn => btn.classList.remove('active'));
