...
// Versão: 5.2 (Otimização Mobile - Final)
...
export async function init(db, user, userData) {
    ...
    function renderGrade(tipo, dia) {
        ...
        const tableBodyHtml = horarios.map((hora, index) => {
            ...
            // Adicionado data-label na célula de hora
            return `<tr class="${rowClass}">${periodoCell}<td class="hour-cell" data-label="Horário">${hora}</td>${celulasProfissionais}</tr>`;
        }).join('');
