...
// Versão Final: Inclui todas as correções de lógica e importação de módulos.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    horariosCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', (e) => {
            ...
            const container = document.getElementById(`container-${periodo.replace('_', '-')}`);
            if (e.target.checked) {
    ...
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
        alert("Lógica de envio a ser implementada! Os dados serão coletados e enviados ao Firestore a partir daqui.");
    });
});